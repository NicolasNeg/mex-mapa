...
**Última actualización:** 2026-05-04 · **FASE 14A**
...
| `/mapa` | `/app/mapa` | **BETA_OPERATIVA_FUERTE (14A)** · `KEEP_LEGACY_BACKUP` | `suscribirMapaPlaza` + `suscribirEstructuraMapa`, fallback cuadre · `guardarNuevasPosiciones` vía DnD | Grid/celdas reales, filtros Ampliados (limbo/taller/en cajón), detalle con enlaces App + legacy, `?q=` sincronizado, scroll a coincidencia; DnD preview/persist con flags `mex.appMapa.dnd` / `mex.appMapa.dndPersist` y rol; `/mapa` **sin redirect** |
...
| Mapa App | Ver flota + `mapa_config`, filtros rápidos, búsqueda global + `?q=`, lista/grid, detalle, copiar MVA/JSON, enlaces incidencias/cuadre/legacy, refresh/resync, DnD con confirmación y persistencia según rol/flags | Editor de estructura, editmap, PDF, altas masivas, flujos radar/chat legacy completos → `/mapa` |
...
- Mapa App: centro operativo beta (14A); editor/layout masivo y herramientas legacy exclusivas permanecen en `/mapa`.
