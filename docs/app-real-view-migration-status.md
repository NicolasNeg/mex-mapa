...
| `/incidencias` | `/app/incidencias` | **REAL_COMPLETA_VISUAL_PORT (15I)** · **APP_FIRST** | `suscribirNotasAdmin`, `guardarNuevaNotaDirecto`, `resolverNotaDirecto`, `eliminarNotaDirecto` | Port visual de bitácora legacy real (header KPI, tabs, filtros, historial/cards, formulario y bloque resolver); ahora incluye **Paridad Total** de operaciones: eliminar nota destructiva, subir adjuntos (imágenes/documentos a Storage) y visualización enriquecida. |
...
| Incidencias | Crear, resolver, subir adjuntos (Storage), eliminar nota, ver evidencias URL/objeto/path, prefill MVA por query | Todo migrado, paridad 100%. |
