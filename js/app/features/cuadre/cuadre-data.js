...
function _normalizeLogItem(id, data = {}, source = 'log') {
  const tipo = String(data.tipo || data.type || source || 'LOG').toUpperCase().trim();
  const timestamp = data.timestamp || data.creadoEn || data.fecha || data._createdAt || 0;
  const posAnterior = String(data.posAnterior || '').trim();
  const posNueva = String(data.posNueva || data.ubicacion || '').trim();
  const details = data.detalles || (posAnterior || posNueva ? `${posAnterior || '-'} → ${posNueva || '-'}` : '');
  return {
    id: String(id || ''),
    source,
    tipo,
    mva: String(data.mva || data.unidad || '').toUpperCase().trim(),
    accion: String(data.accion || data.titulo || data.title || details || '').trim(),
    descripcion: String(data.descripcion || data.description || data.nota || '').trim(),
    detalles: String(details || '').trim(),
    estado: String(data.estado || '').trim(),
    autor: String(data.autor || data.usuario || data.creadoPor || data._createdBy || '').trim(),
    usuario: String(data.usuario || data.autor || '').trim(),
    plaza: String(data.plaza || data.plazaID || data.plazaId || '').toUpperCase().trim(),
    timestamp,
    _sort: _toMs(timestamp)
  };
}

async function _queryCollectionByMva(collectionName, { plaza, mva, limit = 80, source = collectionName } = {}) {
  const mvaUp = String(mva || '').toUpperCase().trim();
  const plazaId = String(plaza || '').toUpperCase().trim();
  if (!mvaUp) return [];

  const eid = _eid();
  const attempts = [];
  let base = db.collection(collectionName).where('mva', '==', mvaUp);
  if (plazaId) attempts.push(base.where('plaza', '==', plazaId));
  if (eid) attempts.push(base.where('empresaId', '==', eid));
  attempts.push(base);

  let lastError = null;
  for (const query of attempts) {
    try {
      const snap = await query.limit(Math.max(1, Number(limit) || 80)).get();
      return snap.docs
        .map(doc => _normalizeLogItem(doc.id, doc.data(), source))
        .filter(item => !plazaId || !item.plaza || item.plaza === plazaId);
    } catch (error) {
      lastError = error;
      const code = String(error?.code || '').toLowerCase();
      const msg = String(error?.message || '').toLowerCase();
      if (code !== 'failed-precondition' && !msg.includes('requires an index')) break;
    }
  }
...
export async function getUnidadBitacora({ plaza, mva, limit = 80 } = {}) {
  ...
  const [patio, logs, notas] = await Promise.allSettled([
    _queryCollectionByMva('historial_patio', { plaza, mva: mvaUp, limit: max, source: 'movimiento' }),
    _queryCollectionByMva(COL.LOGS, { plaza, mva: mvaUp, limit: max, source: 'sistema' }),
    _queryCollectionByMva(COL.NOTAS, { plaza, mva: mvaUp, limit: max, source: 'incidencia' })
  ]);

  const rows = [patio, logs, notas]
    .flatMap(result => result.status === 'fulfilled' ? result.value : [])
    .filter(item => item.mva === mvaUp || String(item.accion || '').toUpperCase().includes(mvaUp))
    .sort((a, b) => Number(b._sort || 0) - Number(a._sort || 0))
    .slice(0, max);
