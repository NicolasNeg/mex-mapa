// ═══════════════════════════════════════════════════════════
//  /js/app/views/cola-preparacion.js — Bridge view
//  La cola completa vive en js/views/cola-preparacion.js.
//  Esta vista es informativa + CTA al módulo completo.
// ═══════════════════════════════════════════════════════════
...
function _html(profile, role) {
  ...
  return _bridge({
    ...
    subtitle:    'Estado de unidades en operación',
    legacyRoute: '/cola-preparacion',
    ctaLabel:    'Abrir cola completa',
    user:        { name, roleLabel, plaza },
    features: [
      ['list_alt',         'Lista de unidades activas'],
      ['pending_actions',  'Estado de preparación'],
      ['timer',            'Tiempos por unidad'],
      ['filter_list',      'Filtros y búsqueda'],
      ['check_circle',     'Confirmación de salidas'],
      ['swap_horiz',       'Cambios de turno'],
    ],
    note: 'La vista completa con control en tiempo real está disponible en el módulo legacy.'
  });
}

// ── Builder compartido (inline — no importado) ────────────────
function _bridge({ icon, iconColor, iconBg, gradFrom, gradTo, accentColor,
...
<div style="padding:28px 24px 56px;max-width:640px;margin:0 auto;font-family:'Inter',sans-serif;">
  ...
  <div style="display:flex;align-items:center;gap:16px;margin-bottom:28px;">
    <div style="width:52px;height:52px;border-radius:16px;background:${iconBg};
  ...
  <!-- Sesión activa -->
  ${user.name ? _userChip(user) : ''}

  <!-- CTA -->
  <div style="background:linear-gradient(135deg,${gradFrom},${gradTo});
    ...
    <p style="font-size:13px;color:rgba(255,255,255,0.65);margin:0 0 20px;line-height:1.6;">
      Accede al módulo completo con todas las funciones operativas.
    </p>
    <a href="${esc(legacyRoute)}"
       style="display:inline-flex;align-items:center;gap:8px;padding:12px 24px;
              background:#fff;color:${gradFrom};border-radius:12px;text-decoration:none;
  ...
  <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:20px;margin-bottom:16px;">
    <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;
                letter-spacing:0.07em;margin-bottom:14px;">Funciones del módulo</div>
...
function _userChip({ name, roleLabel, plaza }) {
  return `
  <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:#fff;
    ...
    <div style="width:32px;height:32px;border-radius:50%;background:${_avatarColor(name)};
                display:flex;align-items:center;justify-content:center;font-size:13px;
                font-weight:800;color:#fff;flex-shrink:0;">
      ${esc(name.slice(0,1).toUpperCase())}
    </div>
    <div style="flex:1;min-width:0;">
      <div style="font-size:13px;font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(name)}</div>
