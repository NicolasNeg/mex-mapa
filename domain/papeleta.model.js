...
export function normalizeMarcasLlantas(p = {}) {
    ...
    const src = (p && typeof p.marcasLlantas === 'object' && p.marcasLlantas)
...
export function resolveZonaFotoPath(zonas = {}, zonaId, papeleta = null) {
    ...
    if (zonaId === 'tablero_kilometraje') {
        return String(papeleta?.fotoTableroPath
...
export function isChecklistComplete(papeleta = {}) {
    ...
    const keysOk = CHECKLIST_KEYS.every((k) => ['ok', 'faltante', 'na'].includes(String(cl[k] || '')));
...
export function isValidFirma(firma) {
    if (!firma || typeof firma !== 'object')
        return false;
    if (firma.rejected === true || firma.singlePoint === true)
        return false;
 ...
 * @param {object} opts
 */
export function createDamageMark({ id, view, x, y, damageType = 'scratch', severity = 'medium', note = '', photoIds = [], source = 'salida', nextDisplayNumber: num, }) {
...
export function puedeEntregar(papeletaOrStatus, optsOrZonas = {}, checklistMaybe) {
    ...
    const hard = [];
    if (!isValidKm(papeleta.salida?.km))
        hard.push('km');
    if (!isGasSet(papeleta.salida?.gas))
        hard.push('gas');
    if (!isChecklistComplete(papeleta))
        hard.push('checklist');
    if (!coreZonasHaveFoto(papeleta.zonas, { papeleta }))
        hard.push('core_photos');
    if (!tableroHaveFoto(papeleta))
        hard.push('tablero_photo');
    if (!isValidFirma(firma))
        hard.push('firma');
    if (opts.pendingWrites)
        hard.push('pending_writes');
    ...
    if (hasFaltantes(papeleta.checklist))
        soft.push('faltantes');
    if (damagesMissingPhoto(papeleta.danosMarcados || papeleta.salida?.danosMarcados || [])) {
        soft.push('damage_photos');
    }
    if (optionalPhotosPending(papeleta.zonas))
        soft.push('optional_photos');
    if (papeleta.correccionesSoloPapeleta)
        soft.push('master_corrected_local');
    if (largeDamagesWithoutVentasReport(papeleta))
        soft.push('large_damage_report');
...
export function computeStatusAfterSave({ status, zonas, checklist, papeleta } = {}) {
    ...
    if (coreZonasHaveFoto(merged.zonas, { papeleta: merged })
